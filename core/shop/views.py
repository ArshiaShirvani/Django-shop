...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["product_images"] = ProductImageModel.objects.filter(product__id=product.id)
        context["is_wished"] = WishListModel.objects.filter(
...
class AddOrRemoveWishListView(LoginRequiredMixin,View):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if product_id:
            try:
                wishlist_item = WishListModel.objects.get(user=request.user,product__id=product_id)
