...
class CartSession:
    ...
    def get_cart_items(self):
        for item in self._cart["items"]:
            product_obj = ProductModel.objects.get(id=item["product_id"], status=ProductStatusType.active.value)
    ...
    def merge_session_cart_in_db(self,user):
        ...
        for item in  self._cart["items"]:
            ...
            cart_item ,created = CartItemsModel.objects.get_or_create(cart=cart,product=product_obj)
            cart_item.quantity = item["quantity"]
            cart_item.save()
